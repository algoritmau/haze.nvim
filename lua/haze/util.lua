local hsluv = require("haze.hsluv")
...
-- local ns = vim.api.nvim_create_namespace("haze")
...
function util.debug(colors)
  colors = colors or require("haze.colors")
  -- Dump unused colors
...
function util.onColorScheme()
  if vim.g.colors_name ~= "haze" then
    vim.cmd([[autocmd! Haze]])
    vim.cmd([[augroup! Haze]])
  end
end
...
function util.autocmds(config)
  vim.cmd([[augroup Haze]])
  vim.cmd([[  autocmd!]])
  vim.cmd([[  autocmd ColorScheme * lua require("haze.util").onColorScheme()]])
  if config.dev then
    vim.cmd([[  autocmd BufWritePost */lua/haze/** nested colorscheme haze]])
  end
